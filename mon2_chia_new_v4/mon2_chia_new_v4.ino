...
void do_stopped()
{
  ...
  while (true) {
    if (readLine(serialBuffer)) {
      ...
      if (inputString.compareTo("+++SLAVE+++") == 0) {
        system_state = PLAYBACK;
        return;
      }
    }
...
//------------------------------------------------------------------------
// PLAYBACK and SLAVE mode
//
//     PLAYBACK and SLAVE modes are similar. In both cases the controller
//     passes steering and throttle data which are used to set the
//     servo and motor controller state. In PLAYBACK mode the controller
//     send 1 update very 0.1 second synchronously. In SLAVE mode data is
//     recieved asynchonously. This code makes no difference between the
//     two cases.
//
//     There are two safety measures:
//
//         1) the throttle value is checked to ensure it is being held
//            open, thus acting as a remote fail-safe or deadman's
//            switch. If the throttle is not open, the robot is stopped
//            and a '-' character is sent to the controller to request
//            it to pause sending information. Once the throttle is re-
//            opened, a '+' will be sent to the controller indicating it
//            can restart.
//         
//         2) if no data is recieved from the controller in over 1
//            second we assume that communications has been lost. We
//            assume this is a non-recoverable error: the robot will be
//            halted and the Arduino will go into a hard loop blinking
//            its on-board LED. A reset will be needed to clear this
//            state.
//     
//------------------------------------------------------------------------
void do_playback()
{
  ...
  Serial.println("System state is now PLAYBACK/SLAVE");

  // Attach interrupt handter to pin 3 to capture the throttle value
  // from the radio reciever which will act as a fail-safe device
  attachInterrupt(1, safetyInterrupt, CHANGE); // interrupt 1 is pin 3
  ...
  // we use a microsecond timer rather than a fixed delay value.
  loop_start_time = micros();
  while (micros() - loop_start_time < 1000000) {
    loop_start_time = micros();
    ...
    if (readLine(serialBuffer)) {
      ...
      // Check the throttle value from the safety interrupt
      if (safety_Result < 1800) {
        power_servo.detach();
        digitalWrite(power_Out, LOW);
        Serial.println("-");  // tell controller to pause            
        Serial.flush();
        while (safety_Result < 1800) {
          // do nothing
          Serial.println(safety_Result);
          Serial.flush();
          delay(1000);
        }
        
        // Restart after safety
        Serial.println("+");            // tell controller to resume
        Serial.flush();
        power_servo.attach(power_Out);  // resume the servo
        loop_start_time = micros();     // reset the loop timer and start again
        continue; 
      }
  ...
  // While loop timed out. Assume lost communication. Panic.
  Serial.println("Playback loop timed out. PANIC.");
  panic();
...
//           times between calls in the 1000 - 2000 range are the ones
